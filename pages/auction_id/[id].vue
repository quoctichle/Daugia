...
const submitBid = async () => {
  if (isBidding.value || auctionStatus.value !== 'Đang đấu giá' || isCurrentUserHighest.value) return

  isBidding.value = true
  isSubmitting.value = true // Also set general submitting for UI consistency

  const bidAmount = nextBidAmount.value

  try {
    const userEmailValue = process.client ? localStorage.getItem('userEmail') : useState('userEmail').value
    if (!userEmailValue) {
      alert('Vui lòng đăng nhập để đấu giá.')
      return // Early exit if no user email
    }
    ...
    const bidsData = [{
      ...
      amount: bidAmount,
      // bidNumber sẽ được tính toán ở backend hoặc không cần thiết nữa
    }]
    ...
    alert(`Bạn đã đấu giá thành công với mức ${formatCurrency(bidAmount)}!`) // Updated alert message
    await Promise.all([loadUserBids(), loadLeaderboard()]) // Refresh both
  } catch (error) {
    alert('Lỗi khi đấu giá: ' + (error.data?.message || error.message))
  } finally {
